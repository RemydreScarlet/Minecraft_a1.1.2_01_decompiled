## Minecraft Alpha 1.1.2_01 地形生成調査の結果

### 0\. 注意書き

この調査結果は殆ど合っていると思われますが、ミス等がある可能性があります。十分承知の上お読みください。mca.pyはこの調査結果を基に作成された地形生成のテスト用スクリプトです。
...
### 7\. 装飾: `cg` (Dungeon)
  ...
  * *(注: `cg` のロジックは `kk` とは無関係なため、今回のmca.py実装には含めない)*
